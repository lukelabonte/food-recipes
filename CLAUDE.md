...
- **`index.html` is auto-generated.** The deploy workflow runs `scripts/generate_index.py` (after `generate_search_index.py`) to build the index page from `recipes.json` and git commit dates. Never edit `index.html` by hand.
...
The index page has a fuzzy search bar powered by Fuse.js (loaded from CDN). The search logic lives in `assets/search.js`. It reads `assets/recipes.json` and filters recipe cards in real time.

`scripts/generate_index.py` produces `index.html` with these required elements (also needed if modifying the template):
